...
class LinearBert(nn.Linear):
  def __init__(self, in_features, out_features, bias=True, std=0.02):
    self.weight = std * rand_truncn(out_features, in_features, dtype=dtypes.float32)# Init as trunc normal
    self.bias = Tensor.zeros(out_features, dtype=dtypes.float32) if bias else None # Init bias as zero
