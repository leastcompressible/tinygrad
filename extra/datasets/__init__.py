...
from tinygrad.helpers import fetch, _cache_dir
...
def fetch_cifar():
  BASE_DIR = f"{_cache_dir}/tinygrad/downloads"
  X_train = Tensor.empty(50000, 3*32*32, device=f'disk:{BASE_DIR}/cifar_train_x', dtype=dtypes.uint8)
  Y_train = Tensor.empty(50000, device=f'disk:{BASE_DIR}/cifar_train_y', dtype=dtypes.int64)
  X_test = Tensor.empty(10000, 3*32*32, device=f'disk:{BASE_DIR}/cifar_test_x', dtype=dtypes.uint8)
  Y_test = Tensor.empty(10000, device=f'disk:{BASE_DIR}/cifar_test_y', dtype=dtypes.int64)

  if not os.path.isfile(f"{BASE_DIR}/cifar_extracted"):
    ...
    open(f"{BASE_DIR}/cifar_extracted", "wb").close()
