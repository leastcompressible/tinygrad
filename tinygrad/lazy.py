...
class LazyBuffer:
  ...
  def const(self, val:Union[float, int], shape:Optional[Tuple[sint,...]]=None, dtype:Optional[DType]=None) -> LazyBuffer:
    shape, dtype = self.shape if shape is None else shape, self.dtype if dtype is None else dtype
    return LazyBuffer.loadop(LoadOps.CONST, tuple(), dtype, self.device, arg=val).reshape((1,)*len(shape)).expand(shape)
