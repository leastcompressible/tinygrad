...
class Tensor:
  ...
  def __init__(self, data:Union[None, int, float, list, LazyBuffer, np.ndarray, bytes], device:Optional[str]=None, dtype:Optional[DType]=None, requires_grad:Optional[bool]=None):
    ...
    if isinstance(data, LazyBuffer): assert dtype is None or dtype == data.dtype, "dtype doesn't match, and casting isn't supported"
      ...
      if data is None: data = LazyBuffer.fromCPU(np.array([], dtype=(dtype or Tensor.default_type).np))
      elif isinstance(data, list): data = LazyBuffer.fromCPU(np.array(data, dtype=(dtype or (dtypes.int32 if all(isinstance(i, int) for i in data) else Tensor.default_type)).np))
  ...
  def log(self): return mlops.Log.apply(self.cast(self.dtype.to_float()))
  def log2(self): return self.log()/math.log(2)
  def exp(self): return mlops.Exp.apply(self.cast(self.dtype.to_float()))
  def exp2(self): return mlops.Exp.apply(self.cast(self.dtype.to_float())*math.log(2))
  def relu(self): return mlops.Relu.apply(self)
  def sigmoid(self): return mlops.Sigmoid.apply(self.cast(self.dtype.to_float()))
  def sqrt(self): return mlops.Sqrt.apply(self.cast(self.dtype.to_float()))
  def rsqrt(self): return (1/self).sqrt()
  def sin(self): return mlops.Sin.apply(self.cast(self.dtype.to_float()))
