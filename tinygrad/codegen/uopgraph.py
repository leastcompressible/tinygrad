...
def float4_expand_load(load, buf, expand):
  if (ex := expand.flatten_add(1)[-1]).op is not UOps.EXPAND: return None
  idx = functools.reduce(UOp.__add__, expand.flatten_add(1)[:-1], UOp.const(dtypes.int, 0))
...
float4_folding = PatternMatcher([
  ...
  # float(2,4) load
  (UOp(UOps.LOAD, src=(UOp.var("buf"), UOp.var("expand"))).name("load"), float4_expand_load),
  # float(2,4) store
