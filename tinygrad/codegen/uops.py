...
def _match(uop:UOp, pat:UPat, store:Dict[str, UOp]) -> bool:
  if pat.name in store:
    if ((known := store[pat.name]).arg, known.dtype, known.arg, known.vin) != (uop.arg, uop.dtype, uop.arg, uop.vin): return False
