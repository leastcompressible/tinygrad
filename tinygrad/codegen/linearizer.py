...
class Linearizer(Kernel):
  ...
  def uop(self, uop:UOps, dtype:Optional[DType]=None, vin:Tuple[UOp, ...]=tuple(), arg:Any=None, cachable=True, insert_before=None, simplify=True) -> UOp:  # noqa: E501
    ...
    if uop == UOps.ALU:
      ...
      elif arg == TernaryOps.WHERE: assert vin[0].dtype == dtypes.bool, f"{arg} selector dtype mismatch {vin[0].dtype=} != {dtypes.bool}"
      # # TODO: enable this
      # elif arg in (BinaryOps.ADD, BinaryOps.SUB, BinaryOps.MUL, BinaryOps.DIV):
  ...
  def ast_parse(self, x:LazyOp, acc: List[UOp], offs:Optional[List[int]], loaded_buffers:Dict[Union[MemBuffer, ConstBuffer, LocalBuffer], List[UOp]], do_reduce=False, loop_ctx=tuple()) -> List[UOp]:  # noqa: E501
    ...
    if x.op in ops:
      ...
      ret = [self.uop(UOps.ALU, dtypes.bool if x.op is BinaryOps.CMPLT else get_lazyop_info(x).dtype, vin=val, arg=x.op) for val in zip(*values)]
    return ret
