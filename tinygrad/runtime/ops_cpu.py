...
numpy_fxn_for_op: Dict[Op, Callable] = {
  ...
  BinaryOps.MAX: np.maximum, BinaryOps.CMPLT: lambda x,y: (x<y).astype(output_type(x,y)), BinaryOps.ADD: lambda x, y: np.add(x, y),
  BinaryOps.SUB: lambda x, y: np.subtract(x, y), BinaryOps.MUL: lambda x, y: np.multiply(x, y),
