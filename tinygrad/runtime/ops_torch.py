...
torch_fxn_for_op: Dict[Op, Callable] = {
  ...
  BinaryOps.ADD: lambda x,y: torch.add(x,y), BinaryOps.SUB: lambda x,y: torch.sub(x,y),
  BinaryOps.MUL: lambda x,y: torch.mul(x,y), BinaryOps.DIV: lambda x,y: torch.div(x,y).type(torch.promote_types(x.dtype, y.dtype)),
