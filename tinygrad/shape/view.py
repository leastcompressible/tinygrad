...
class View:
  ...
  def unbind(self) -> Tuple[View, Dict[Variable, int]]:
    ...
    new_mask = tuple((a if isinstance(a, int) else a.substitute(unbound_vars), b if isinstance(b, int) else b.substitute(unbound_vars)) \
                     for (a, b) in self.mask) if self.mask is not None else None
  ...
  def stride(self, mul: Tuple[int, ...]) -> View:
    ...
    mask = tuple([(((mx if m > 0 else s-my)+(abs(m)-1))//abs(m), ((my if m > 0 else s-mx)+(abs(m)-1))//abs(m)) for (mx,my),s,m in \
                  zip(self.mask, self.shape, mul)]) if self.mask is not None else None
