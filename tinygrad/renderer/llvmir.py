...
def uops_to_llvm_ir(function_name:str, uops:List[UOp]) -> Tuple[str, Dict]:
  ...
  args = [dtype_to_llvm_dtype[dtype].as_pointer() if isinstance(dtype, PtrDType) else dtype_to_llvm_dtype[dtype] for dtype in buf_to_dtype.values()]
  func = ir.Function(module, ir.FunctionType(ir.VoidType(), args), name=function_name)
  for a in func.args:
