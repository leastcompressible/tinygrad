...
from tinygrad.helpers import flatten, merge_dicts, DEBUG, Context, GRAPH, BEAM, getenv, all_int, GraphException, colored, JIT, ContextVar
...
IN_JIT_CONTEXT = ContextVar('IN_JIT_CONTEXT', 0)
class TinyJit(Generic[ReturnType]):
  ...
  def __call__(self, *args, **kwargs) -> ReturnType:
    ...
    if not JIT or IN_JIT_CONTEXT or self.cnt == 0:
      # jit ignore
      with Context(BEAM=0 if getenv("IGNORE_JIT_FIRST_BEAM") else BEAM.value, IN_JIT_CONTEXT=1):
      ...
      with Context(GRAPH=getenv("JITGRAPH", GRAPH.value), BEAM=getenv("JITBEAM", BEAM.value), IN_JIT_CONTEXT=1):
