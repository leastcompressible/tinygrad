...
def optimize_schedule(schedule:List[ScheduleItem]) -> List[ScheduleItem]:
  # Building dep graph
  r_dep = defaultdict(list)
  w_dep:Dict[LazyBuffer,int] = {}
  graph = defaultdict(list) # children for a node
  for i,si in enumerate(schedule):
    for lb in si.inputs:
      if lb in w_dep: graph[w_dep[lb]].append(i)
      r_dep[lb].append(i)
    if si.outputs[0] in r_dep: 
      for j in r_dep.pop(si.outputs[0]): graph[j].append(i)
    if si.outputs[0] in w_dep: graph[w_dep[lb]].append(i)
    w_dep[si.outputs[0]] = i

  prios:Dict[int,int] = {} # prio for each node. the less prio, the closer to the beggining.
  def _dfs(v:int, transfers_before=0) -> int: # weight
